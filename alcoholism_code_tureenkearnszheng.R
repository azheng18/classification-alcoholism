setwd("/Users/anthonyzheng/Documents/Data Science Portfolio/Alcoholism Classification")
...
summary(as.factor(heavydrinker))
...
heavydrinker_data = analysis_data[which(heavydrinker==1),]
nonheavydrinker_data = analysis_data[which(heavydrinker==0),]

#Comparison by sex/gender
summary(as.factor(heavydrinker_data$sex))
summary(as.factor(nonheavydrinker_data$sex))

#Comparison by reason why the student attends the university
summary(as.factor(heavydrinker_data$reason))
summary(as.factor(nonheavydrinker_data$reason))
summary(as.factor(analysis_data$reason))
...
X_matrix <- model.matrix(heavydrinker ~ ., X_Data) #ideally we want this to make qualitative vars
